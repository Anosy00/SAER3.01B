...
ping -c 1 192.168.16.2  | grep "1 received" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "[TEST] Ping propre réseau = Accepté"
else
    echo "[TEST] Ping propre réseau = Refusé" 
...
nping 192.168.32.1 --tcp -p 1234  | grep "RCVD"  > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "[TEST] Employé -> Compta port 1234 = Accepté"
else
    echo "[TEST] Employé -> Compta port 1234 = Refusé" 
fi

nping 192.168.32.1 --tcp -p 22  | grep "RCVD" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "[TEST] Employé -> Compta port 22 = Accepté"
else
    echo "[TEST] Employé -> Compta port 22 = Refusé"
fi

nping 172.12.150.1 --tcp -p 587  | grep "RCVD" > /dev/null 2>&1
if [ $? -eq 0 ]; then
     echo "[TEST] Employé -> Serveur S 587 = Accepté"
else
    echo "[TEST] Employé -> Serveur S 587 = Refusé"
fi